...
const hydrateArray = (fields, type, docKey, value) => {
  return value
    .map((item, index) => {
      const field = klona(dotProp.get(fields, `${docKey}._template`));
      return Object.keys(field).filter(key => !key.includes('_'))
        .reduce((obj, key) => {
          dotProp.set(obj, `${key}.${type}`, item[key]);
          obj[key]._attributes.name = obj[key]._attributes.name.replace(/\.\$index\./, `.${index}.`);
          return obj;
        }, field);
    });
}
...
const hydrate = (fields, doc, type) => {
  ...
  Object
    ...
    .map(key => ({
      ...
      multiple: dottedFields[key].match(/\.\$index\./)
    }))
    .forEach(({key, multiple}) => {
      const docKey = key.replace(/\.\$index\..*$/, '');
      const value = dotProp.get(doc, docKey);
      if (!value) return;

      if (multiple) {
        dotProp.set(fields, `${docKey}._items`, hydrateArray(fields, type, docKey, value));
      } else {
