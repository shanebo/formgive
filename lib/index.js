...
function create({ type, props, arrayItemType }) {
  ...
  function buildObjectField(prop, key, value, parentKey, scope) {
    ...
    // Use face name when .field({ name }) overrides the schema key (FormData control name)
    enrichField(obj, obj.name, value);
    return obj;
  }
  ...
  function enrichField(obj, name, value) {
    if (obj.options) {
      ...
      obj.itemOptions = { ...(obj.itemOptions || {}) };
      obj.itemOptions.component ??= 'Choice';
      obj.itemOptions.type ??= 'chip';
  ...
  function schema(input = {}, parentKey = '', scope = null) {
    ...
    if (props && hasKeys(props)) {
      if (parentKey && result.component === undefined) {
        result.component = 'Fieldset';
