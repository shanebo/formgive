...
const FIELD_KEYS = ['input', 'name', 'label', 'class', 'classes', 'step', 'min', 'max', 'prefix', 'placeholder', 'legend', 'autocapitalize', 'autocomplete', 'form', 'itemOptions', 'multiple', 'autofocus', 'hideOptional', 'fieldVariant', 'variant', 'checkedVariant', 'selectedVariant', 'activeVariant', 'attributes', 'help', 'iconBefore', 'iconAfter'];

const COPY_CONFIG_EXCLUDE = new Set(['props', 'type']);
...
function create({ type, props, arrayItemType }) {
  ...
  function createResolveContext(value, scope) {
    ...
    const coercedValue = value != null && inputType !== config.type && defaultCoercions[config.type]
      ? defaultCoercions[config.type](value, inputType)
      : value;
  ...
  function coerceValue(value, scope) {
    ...
    const { type, computed, transform, nullable, required, fallback } = config;
    ...
    if (value === '' && type !== 'array' && !required && !nullable && fallback === undefined) {
      return undefined;
    }

    if (inputType === type || value === undefined || value === null) {
      return value;
    }

    const coercionFn = defaultCoercions[type];
    return coercionFn
  ...
  function data(input, opts = {}, scope = null) {
    ...
    const { type, props, fallback } = config;
    ...
    if (type === 'array' && arrayItemType && Array.isArray(coercedInput)) {
  ...
  function modifiers(input, key, scope = null) {
    ...
    const { type, modify, props } = config;
  ...
  function schema(input = {}, parentKey = '', scope = null) {
    ...
    const { type, props, name, label } = config;
    const result = { ...config };
  ...
  function validateValue(value) {
    ...
    const { type, nullable, required, pattern, test, positive, negative, integer, float, min, max, minlength, length, maxlength, floatMinDecimals, floatMaxDecimals, optionValues, optionsSet } = config;
    ...
    if (value === null) {
      return nullable ? null : 'cannot be null';
    }
