...
function create(type, props) {
    ...
    if (config.props) {
      return Object
        ...
        .reduce((output, [key, prop]) => {
          output[key] = prop.data(input?.[key]);
          return output;
        }, { ...input });
    }
  ...
  function parse(input) {
    ...
    // const errors = process.env.NODE_ENV === 'development' ? false : validate(result, 'top level');
    const errors = validate(result, 'top level');
...
const formgive = {
  ...
  integer: () => create('number').integer(),
  float: (minDecimals, maxDecimals) => create('number').float(minDecimals, maxDecimals),
  object: (props) => create('object', props),
  array: (type) => {
    // console.log('array type before create', type);

    // if (type === undefined) {
    //   type = create('mixed', ['string', 'null', 'boolean', 'number', 'undefined']);
    // }

    // console.log('array type after create', type);
    return create('array', type?.props ? type.props : undefined);
    // return create('array', type.props ? type.props : type);
  },
  boolean: () => create('boolean'),
  mixed: (types) => create(types),
