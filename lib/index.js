...
const hydrate = (fields, doc, type) => {
  ...
  const keys = Object
    ...
    .forEach(({key, multiple}) => {
      ...
      // if (multiple && !dotProp.has(fields, key)) {
      //   copyMultipleField(fields, key, multiple[1]);
      // }

      if (multiple) {
        const docKey = key.replace(/\.\d+\..*$/, '');
        const values = dotProp.get(doc, docKey)
        .forEach((item, index) => {
          if (index === 0) {
            Object.keys(item).filter(key => !key.includes('_')).forEach(key => {
              dotProp.set(fields, `${docKey}.0.${key}.${type}`, item[key]);
            });
          } else {
            const dupe = klona(dotProp.get(fields, `${docKey}.0`));
            dotProp.set(fields, `${docKey}.${index}`, dupe);

            Object.keys(dupe).filter(key => !key.includes('_')).forEach(key => {
              dotProp.set(fields, `${docKey}.${index}.${key}.${type}`, item[key]);
              const name = dotProp.get(fields, `${docKey}.${index}.${key}._attributes.name`);
              dotProp.set(fields, `${docKey}.${index}.${key}._attributes.name`, name.replace(/\.\d+\./, `.${index}.`));
            });
          }
        });
      } else {
        const value = dotProp.get(doc, key);
        if (value) {
          dotProp.set(fields, `${key}.${type}`, value);
        }
      }
