...
function create({ type, props, arrayItemType }) {
  ...
  function data(input, opts = {}) {
    if (config.props) {
      // console.log('deal with object');
      // console.log({ input, props: config.props });
      const obj = Object
        ...
        .reduce((output, [key, prop]) => {
          ...
          // if (input?.[key] === '' && !prop.config.required && !prop.config.nullable && prop.config.fallback === undefined) {
          //   console.log('deleting key because it is empty');
          //   delete output[key];
          // }
    ...
    // if (coercedInput === '' && !config.nullable && !config.required && config.fallback === undefined) {
    //   return undefined;
    // }

    // console.log({ coercedInput });
