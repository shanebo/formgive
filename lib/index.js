...
function create({ type, props, arrayItemType }) {
  ...
  function createResolveContext(value, input = value) {
    return { value, input };
  }


  function isGoneSchema(schema) {
    return schema?.config?.type === 'gone';
  }
  ...
  function resolve(value, rootInput = value) {
    if (typeof config.predicate !== 'function') {
      return schemaApi;
    }

    const ctx = createResolveContext(value, rootInput);
    const branch = config.predicate(ctx)
      ? config.thenBranch
      : config.elseBranch;
    ...
    return resolved === schemaApi
      ? schemaApi
      : resolved.resolve(value, rootInput);
  }
  ...
  function coerceValue(value, rootInput = value) {
    const resolved = resolve(value, rootInput);

    if (resolved !== schemaApi) {
      return resolved.coerceValue(value, rootInput);
    }
  ...
  function data(input, opts = {}, rootInput = input) {
    const resolved = resolve(input, rootInput);

    if (resolved !== schemaApi) {
      return resolved.data(input, opts, rootInput);
    }
    ...
    if (config.props) {
      ...
      for (const [key, prop] of Object.entries(config.props)) {
        ...
        const value = prop.data(input?.[key], opts, rootInput);
    ...
    const coercedInput = coerceValue(input, rootInput);

    if (config.type === 'array' && arrayItemType && Array.isArray(coercedInput)) {
      return coercedInput
        .flatMap((item) => {
          const parsedItem = arrayItemType.data(item, opts, rootInput);
  ...
  function modifiers(input, key, rootInput = input) {
    const resolved = resolve(input, rootInput);

    if (resolved !== schemaApi) {
      return resolved.modifiers(input, key, rootInput);
    }
    ...
    return [config.modify && config.modify(key, input)]
      .concat(Object.entries(config.props || {}).map(([childKey, prop]) => {
        const value = prop.data(input?.[childKey], {}, rootInput);
        return prop.modifiers(value, childKey, rootInput);
      }))
      .filter(Boolean)
      .join(' ');
  }
  ...
  function buildObjectField(prop, key, value, parentKey, rootInput) {
    const name = parentKey ? `${parentKey}.${key}` : key;
    const id = makeId(name.replace(/\./g, '-'));
    const resolvedProp = prop.resolve(value, rootInput);
    ...
    const obj = {
      propType: resolvedProp.config.type,
      key,
      id,
      name,
      label: humanize(key),
      ...resolvedProp.schema(value, key, rootInput),
  ...
  function schema(input = {}, parentKey = '', rootInput = input) {
    const resolved = resolve(input, rootInput);

    if (resolved !== schemaApi) {
      return resolved.schema(input, parentKey, rootInput);
    }
    ...
    if (config.type === 'array') {
      if (Array.isArray(input)) {
        ...
        result.items = input.map((item, i) => {
          ...
          const itemSchema = arrayItemType.schema(item, itemKey, rootInput);
    ...
    if (config.props && hasKeys(config.props)) {
      ...
      result.props = Object
        ...
        .reduce((props, [key, prop]) => {
          const value = prop.data(input?.[key], {}, rootInput);
          const field = buildObjectField(prop, key, value, parentKey, rootInput);
  ...
  function validate(input, key, opts = {}, rootInput = input) {
    const resolved = resolve(input, rootInput);

    if (resolved !== schemaApi) {
      return resolved.validate(input, key, opts, rootInput);
    }
    ...
    if (config.type === 'array' && Array.isArray(input)) {
      const arrayErrors = input
        .map((item, index) => arrayItemType.validate(item, `${key}[${index}]`, opts, rootInput))
        .filter(Boolean);
    ...
    const errors = Object
      ...
      .map(([propKey, prop]) => [propKey, prop.validate(input?.[propKey], propKey, opts, rootInput)])
      .filter(([, error]) => error)
      .reduce((acc, [propKey, error]) => ({ ...acc, [propKey]: error }), {});
  ...
  function parse(input, opts = {}) {
    const resolved = resolve(input, input);
