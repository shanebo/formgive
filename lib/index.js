...
const expandShorthand = (str, options) => {
  ...
  const input = str.replace(/\*|!|([@][a-zA-Z]*)/g, (match) => {
    const matchSettings = defs.SHORTHAND_FLAGS[match] || defs.SHORTHAND_FLAGS[match.charAt(0)](match.substr(1));
    // because replace loops
    // this builds up field
    // according to all shorthand flags
    field = merge(field, matchSettings);
    return '';
  });

  return merge(field, extendInput(input, {}, options, field._attributes));
}


const extendInput = (name, def = {}, options, attributes) => {
  const defFn = defs[name];

  if (defFn) {
    def = merge(defFn(options, attributes), def);
    if (def._input !== name) {
...
const mapOptions = (type, key, options, name) => options.map((option, o) => {
    ...
    if (option.disabled) {
      obj._attributes.disabled = option.disabled;
    }
    if (option.default) {
