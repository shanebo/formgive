...
function create({ type, props, arrayItemType }) {
  ...
  const defaultCoercions = {
    ...
    integer: (v, type) => {
      if (type === 'string') {
        ...
        const result = isNaN(parsed) ? v : parsed;
        console.log({ result });
        return result;
      }
  ...
  //   // Handle null/undefined input for nullable fields
  //   if ((input === null || input === undefined) && config.nullable) {
  //     console.log('this is nullable');
  //     return null;
  //   }

  //   if (config.type === 'object' && typeof input === 'object') {
  //     return Object.entries(config.props).reduce((output, [key, prop]) => {
  //       let value = prop.data(prop.coerceValue(input?.[key]), opts);
  //       if (value !== undefined) {
  //         output[key] = value;
  //       }
  //       return output;
  ...
  //   if (config.type === 'array' && Array.isArray(input) && arrayItemType) {
  //     return input.map(item => arrayItemType.data(arrayItemType.coerceValue(item), opts));
  //   }
    ...
    if (config.props) {
      return Object
        ...
        .reduce((output, [key, prop]) => {
          ...
          // const value = prop.data(coerceValue(input?.[key], opts));
          if (value !== undefined) {
            output[key] = value;
          }
          return output;
...
// WORKS
  ...
  //   if (config.type === 'array' && arrayItemType && Array.isArray(input)) {
  //     return input.map(item => arrayItemType.data(arrayItemType.coerceValue(item), opts));
  //   }
  ...
  //   if (config.type === 'array' && Array.isArray(coercedInput) && arrayItemType) {
  //     return coercedInput.map(item => arrayItemType.data(item, opts));
  //   }

  //   if (config.props) {
  //     return Object.entries(config.props).reduce((output, [key, prop]) => {
  //       const value = prop.data(coercedInput?.[key], opts);
  //       if (value !== undefined) {
  //         output[key] = value;
  //       }
  //       return output;
  //     }, opts.strict ? {} : { ...coercedInput });
  //   }
